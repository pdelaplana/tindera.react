...
import { useAddModifier, useDeleteModifier, useUpdateGlobalModifier } from '@/hooks';
...
export const GlobalModifierFormModal: React.FC<GlobalModifierFormModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen) {
      const hasInventoryLink = !!initialData?.inventory_item_id;
      reset(
        initialData
          ? {
  ...
  const onSubmit = async (data: ModifierFormData) => {
    console.log('Submitting modifier data:', isEditMode);
