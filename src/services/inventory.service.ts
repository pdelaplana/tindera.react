...
export const inventoryService = {
  ...
  /**
   * Get a single inventory transaction by ID with user profile and package size information
   */
  async getInventoryTransaction(transactionId: string): Promise<
    ApiResponse<
      InventoryTransaction & {
  ...
  > {
    try {
      ...
      let packageSize:
        ...
        | undefined;
      if (transaction.package_size_id) {
        const { data: pkgData } = await supabase
          .from('package_sizes')
          .select('package_name, package_uom, units_per_package')
          .eq('id', transaction.package_size_id)
          .single();
  ...
  async createReceiptTransaction(
    ...
    try {
      ...
      if (transactionData.package_size_id && transactionData.package_quantity) {
        ...
        // Calculate base quantity: package_quantity × units_per_package
        finalQuantityIn = transactionData.package_quantity * packageSize.units_per_package;

        // Calculate unit cost: package_cost ÷ units_per_package
        finalUnitCost =
          (transactionData.package_cost_per_unit || 0) / packageSize.units_per_package;
  ...
  async createSaleTransaction(
    ...
    try {
      ...
      const { data: invItem, error: fetchError } = await supabase
        .from('inventory_items')
        .select('current_count, unit_cost')
        .eq('id', transactionData.item_id)
        .single();

      if (fetchError) {
        logger.error(new Error(fetchError.message), {
          context: 'createSaleTransaction_fetchItem',
        });
      ...
      if (!invItem) {
        ...
        logger.error(error, { context: 'createSaleTransaction_fetchItem' });
        return { data: null, error };
      }

      const newCount = (invItem.current_count || 0) - transactionData.quantity;
      ...
      if (txnError) {
        logger.error(new Error(txnError.message), { context: 'createSaleTransaction' });
        return { data: null, error: new Error(txnError.message) };
      }
      ...
      if (updateError) {
        logger.error(new Error(updateError.message), {
          context: 'createSaleTransaction_updateItem',
        });
        return { data: null, error: new Error(updateError.message) };
      }
      ...
      const error = err as Error;
      logger.error(error, { context: 'createSaleTransaction' });
      return { data: null, error };
    }
  },
